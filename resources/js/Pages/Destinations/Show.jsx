...
export default function DestinationShow({ destination, hotels }) {
    ...
    // Filter and sort hotels client-side (matching Results.jsx logic)
    const filteredAndSortedHotels = useMemo(() => {
        const allHotels = hotels?.data || [];
        if (allHotels.length === 0) return [];

        let filtered = [...allHotels];

        // Apply pool vibe filter
        if (filters.poolVibe) {
        ...
        // Apply sorting - Premium hotels always first
        filtered.sort((a, b) => {
    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-slate-50/60 font-sans">
                ...
                {/* Header Banner - matching Results.jsx */}
                    ...
                    {hotels.data.length === 0 && activeFilterCount === 0 ? (
                        <div className="text-center py-8 sm:py-12 md:py-16 lg:py-20 bg-white rounded-2xl ring-1 ring-slate-200/70 shadow-[0_1px_2px_rgba(15,23,42,0.03)]">
                            ...
                            <p className="text-slate-500 mb-4 sm:mb-6 md:mb-8 leading-relaxed max-w-xs sm:max-w-sm md:max-w-md mx-auto px-3 sm:px-4 text-xs sm:text-sm md:text-base">
                                There are no hotels listed in {destination.name} yet.
                            </p>
                            <Link
                                href="/destinations"
                                    ...
                                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
                                        ...
                                        <div className="flex items-center gap-2 sm:gap-4 w-full sm:w-auto justify-between sm:justify-end">
                                            <span className="text-xs sm:text-sm font-semibold text-slate-700 bg-slate-100 ring-1 ring-inset ring-slate-200/60 px-3 py-1 rounded-full">
                                                {activeFilterCount > 0 
                                                    ? `${filteredAndSortedHotels.length} of ${hotels?.total || 0}`
                                                    : `${hotels?.total || filteredAndSortedHotels.length}`
                                                } hotel{(activeFilterCount > 0 ? filteredAndSortedHotels.length : (hotels?.total || filteredAndSortedHotels.length)) !== 1 ? 's' : ''}
                                            </span>
                                ...
                                {filteredAndSortedHotels.length > 0 ? (
                                    ...
                                    <div className="text-center py-12 sm:py-16 md:py-20 bg-white rounded-2xl ring-1 ring-slate-200/70 shadow-[0_1px_2px_rgba(15,23,42,0.03)] transition-all duration-300 animate-fadeIn">
                                        <div className="relative inline-block mb-4 sm:mb-6">
                                            <svg className="w-16 h-16 sm:w-20 sm:h-20 mx-auto text-slate-300" viewBox="0 0 24 24" fill="currentColor">
                                                <path d="M15.5 14h-.79l-.28-.27A6.471 6.471 0 0016 9.5 6.5 6.5 0 109.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/>
                                            </svg>
                                            <div className="absolute -top-1 -right-1 w-7 h-7 sm:w-8 sm:h-8 bg-orange-50 ring-1 ring-inset ring-orange-100 rounded-full flex items-center justify-center">
                                        ...
                                        <p className="text-sm sm:text-base text-slate-500 mb-6 sm:mb-8 max-w-sm mx-auto px-4 leading-relaxed">
                                            None of the {hotels?.total || 0} hotels in {destination.name} match your current filter combination. Try adjusting or clearing your filters.
                                        </p>
                                ...
                                {hotels?.links?.length > 3 && (
                                    <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
                                        <p className="text-sm text-slate-500">
                                            Showing <span className="font-semibold text-slate-900">{hotels.from}</span> to{' '}
                                            <span className="font-semibold text-slate-900">{hotels.to}</span> of{' '}
                                            <span className="font-semibold text-slate-900">{hotels.total}</span> results
                                        </p>
        ...
        <div className={cardClasses}>
            <Link href={`/hotels/${hotel.slug}`} className="block">
                ...
                <div className="p-3 sm:p-4 md:p-5 lg:p-6 bg-white">
                    ...
                    <p className="text-[10px] sm:text-xs text-neutral-500 mb-3 sm:mb-4 md:mb-5 font-light tracking-wide uppercase">{destination.name}</p>
