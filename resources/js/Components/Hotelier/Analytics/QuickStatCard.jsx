export default function QuickStatCard({ title, views, clicks, color }) {
    ...
    return (
        <div className={`${colors.bg} rounded-xl p-4`}>
            <div className={`text-xs font-semibold uppercase ${colors.icon} mb-2`}>{title}</div>
            <div className="space-y-2">
                ...
                <div className="flex items-center gap-3">
                    <div>
                        <div className="text-lg font-semibold text-gray-700">{clicks.toLocaleString()}</div>
                        <div className="text-xs text-gray-500">Clicks</div>
                    </div>
                    <div className="text-right ml-auto">
                        <div className={`text-lg font-semibold ${colors.icon}`}>{ctr}%</div>
