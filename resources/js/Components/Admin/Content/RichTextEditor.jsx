...
// ─── Resizable Image Node View (React) ──────────────────────────────────────
const ResizableImageComponent = ({ node, updateAttributes, selected }) => {
    ...
    return (
        <NodeViewWrapper className="relative inline-block my-2" style={{ width: node.attrs.width ? `${node.attrs.width}px` : 'auto', maxWidth: '100%' }}>
            <img
                ref={imgRef}
                src={node.attrs.src}
                alt={node.attrs.alt || ''}
...
const ToolbarButton = ({ onClick, isActive, title, children, disabled }) => (
    <button
        ...
        disabled={disabled}
        title={title}
        className={`p-1.5 sm:p-2 rounded text-sm transition-colors ${
            isActive
...
const EditorToolbar = ({ editor }) => {
    ...
    const handleImageUpload = useCallback((e) => {
        ...
        fetch(route('admin.content.upload-image'), {
            ...
            .catch(() => {
                // Fallback: load as base64 data URL for local preview
    ...
    return (
        <>
            <div className="flex flex-wrap items-center gap-0.5 sm:gap-1 px-2 sm:px-3 py-1.5 sm:py-2 border-b border-gray-200 bg-gray-50/80 rounded-t-lg">
                {/* Heading / Font Size */}
                <select
                ...
                >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M10 6h11M10 12h11M10 18h11M3 5l2 1V4M3 11h2l-1.5 2H6M3 17h1.5l.5 1-.5 1H3" />
                    </svg>
                </ToolbarButton>
                ...
                >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M13.19 8.688a4.5 4.5 0 011.242 7.244l-4.5 4.5a4.5 4.5 0 01-6.364-6.364l1.757-1.757m9.86-2.03a4.5 4.5 0 00-1.242-7.244l-4.5-4.5a4.5 4.5 0 00-6.364 6.364l1.757 1.757" />
                    </svg>
                </ToolbarButton>
                ...
                {/* Embed Media */}
                <ToolbarButton onClick={handleEmbed} title="Embed video (YouTube, Vimeo)">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
