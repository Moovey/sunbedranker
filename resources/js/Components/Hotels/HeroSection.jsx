...
// ============================================
export function ImageGallery({ mediaItems, activeImageIndex, hotelName, onPrevImage, onNextImage, setActiveImageIndex, isPremium = false }) {
        ...
        <div className="lg:col-span-2 flex flex-col">
            <div className={`relative ${heightClass} rounded-2xl overflow-hidden shadow-xl ring-1 ring-black/5 border border-gray-100 bg-black`}>
                {isActiveVideo ? (
                    ...
                    <img
                        src={active?.url || '/images/default-hotel.jpg'}
                        alt={hotelName}
                ...
                {items.length > 1 && (
                    <>
                        ...
                        <div className="absolute top-3 sm:top-4 right-3 sm:right-4 bg-black/55 ring-1 ring-white/10 text-white px-2.5 py-1 rounded-full text-[11px] sm:text-xs font-semibold tracking-wide backdrop-blur-md z-20">
                            ...
                            >
                                ...
                                {isVideoThumb && (
                                    <div className="absolute inset-0 flex items-center justify-center bg-black/30">
                                        <div className="w-8 h-8 sm:w-9 sm:h-9 bg-white/90 rounded-full flex items-center justify-center shadow">
...
export function HeroSection({ hotel, allImages, mediaItems, activeImageIndex, onPrevImage, onNextImage, onBookingClick, setActiveImageIndex }) {
    ...
    // Fall back to image-only items if mediaItems wasn't provided (for older callers).
    const items = (mediaItems && mediaItems.length > 0)
        ? mediaItems
        : (allImages || []).map((url) => ({ type: 'image', url, thumbnail: url }));
