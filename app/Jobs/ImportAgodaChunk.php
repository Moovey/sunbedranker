...
class ImportAgodaChunk implements ShouldQueue
{
    ...
    protected int $chunkSize = 10000; // rows per job

    public function __construct(string $filePath, int $offset)
    {
    ...
    public function handle(): void
    {
        ...
        $csv = Reader::from($this->filePath);
        $csv->setHeaderOffset(0);

        $header = $csv->getHeader();
        $header = array_map(fn ($h) => strtolower(trim($h)), $header);

        // Count total only on first chunk
        $totalRecords = (int) Cache::get('agoda_import_total', 0);
        if ($totalRecords === 0) {
            $totalRecords = $csv->count();
            Cache::put('agoda_import_total', $totalRecords, now()->addHours(4));
        }

        $this->updateProgress('running', $this->offset, $totalRecords, "Processing rows {$this->offset} - " . min($this->offset + $this->chunkSize, $totalRecords) . " of {$totalRecords}...");
        ...
        $newOffset = $this->offset + $processed;

        // Dispatch next chunk or finish
        if ($processed >= $this->chunkSize && $newOffset < $totalRecords) {
            $this->updateProgress('running', $newOffset, $totalRecords, "Processed {$newOffset} / {$totalRecords}...");
            ImportAgodaChunk::dispatch($this->filePath, $newOffset);
        } else {
            $this->updateProgress('completed', $newOffset, $totalRecords, "Import complete. {$newOffset} hotels processed.");
            Log::info('AgodaDirectory import complete', ['total' => $newOffset]);
            Cache::forget('agoda_import_total');

            // Clean up temp CSV
            if (str_contains($this->filePath, 'agoda-import.csv') && file_exists($this->filePath)) {
                unlink($this->filePath);
                Log::info('AgodaDirectory: cleaned up temp file');
            }
        }
    }
